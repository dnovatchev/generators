import module namespace f = "http://www.w3.org/2005/xpath-functions-2025/generator" at "generator-functions.xqm" ;
...
"=====================",    
   ...
   "=================",
  (
   let $gn1 := $gen2ToInf => gn:filter(fn($n){$n mod 2 eq 0}),
       $gn2 := $gen2ToInf => gn:filter(fn($n){$n mod 3 eq 0}),
       $gn3 := $gen2ToInf => gn:filter(fn($n){$n mod 5 eq 0}),
       $genOfGens := gn:make-generator-from-array([$gn1, $gn2, $gn3])
    return
       gn:merge-sorted-generators($genOfGens) => gn:subrange(1, 40) => gn:to-array() 
  ), 
